...
scans_path = '../data/images'

demented_path = "../data/images/demented"
nondemented_path = "../data/images/nondemented"
...
for scan_path in glob.glob(os.path.join(scans_path, 'OAS2_*')):
    ...
    for epi in glob.glob(os.path.join(scan_path, 'RAW', '*.epi')):
        for image in glob.glob(os.path.join(epi, '*.png')):
            i += 1
            print(image)
